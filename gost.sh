...
download_gost(){
	...
	mkdir /root/.gost
	wget --no-check-certificate https://okdns.cc/gost/config.json.example -O /root/.gost/config.json
	echo -e "${Info} gost 主程序安装完毕！开始配置服务文件..."
}

service_gost(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://okdns.cc/gost/gost_centos.service -O /etc/init.d/gost; then
		...
		if ! wget --no-check-certificate https://okdns.cc/gost/gost_debian.service -O /etc/init.d/gost; then
			echo -e "${Error} gost服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/gost
